...
public:
    ...
    double getCmdZ(double delay)
    {
      ros::Time time_now = ros::Time::now();
      double t_cur = (time_now - start_time).toSec() + delay;

      if (t_cur > 0.0 && t_cur <= traj_duration)
      {
        // std::cout<<"cmd height="<<minco_traj.getPos(t_cur)[2]<<std::endl;
        double z = std::min(std::max(minco_traj.getPos(t_cur)[2], 0.48), 0.64);
        return (z-0.48)/0.16;
      }
      else
        return 1.0;
    ...
    std::vector<TrajPoint> getRefPoints(const int T, double dt)
    {
      ...
      for (double t=t_cur+dt; j<T; j++, t+=dt)
      {
        ...
        if (temp <= traj_duration)
        {
          ...
          tp.y = po[1];
          // tp.z = po[2];
          tp.z = std::min(std::max(po[2], 0.48), 0.64);
          tp.z = (tp.z - 0.48)*0.16;
