        ...
        if (t_cur <= traj_duration_ && t_cur >= 0.0)
        {
            ...
            for (int i=0; i<T; i++)
            {
                ...
                if (t_temp < upv && t_temp > 0.0)
                {
                    ...
                    xref(2, i) = max(0.7*max_speed * t_temp / upv, max_speed*0.1);
                }
                else if (t_temp < downvt)
                {
                    temp = csp.get_state(t_temp);
                    xref(2, i) = max_speed*0.7;
                }
                else if (t_temp < traj_duration_)
                {
                    temp = csp.get_state(t_temp);
                    xref(2, i) = 0.7*max_speed * (traj_duration_ - t_temp) / (traj_duration_ - downvt);
                }
                else
                {
...
void MPC::getCmd(void)
{
    ...
    if (control_a)
    {
        ...
        cmd.height = traj_analyzer.getCmdZ(0.0);
    }
    else
    {
