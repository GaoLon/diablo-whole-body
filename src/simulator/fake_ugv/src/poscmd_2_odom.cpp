...
double max_height = 0.64;
double min_height = 0.48;
...
void simCallback(const ros::TimerEvent &e)
{
	...
	new_odom.pose.pose.position.z  = std::max(std::min(diablo_cmd.height, 1.0), 0.0) * (max_height-min_height) + min_height;
...
{        
	...
	nh.getParam("simulator/init_x", p_init_x);
	nh.getParam("simulator/init_y", p_init_y);
	nh.getParam("simulator/init_z", p_init_z);
	nh.getParam("simulator/max_height", max_height);
	nh.getParam("simulator/min_height", min_height);
